...
{
		ROS_WARN_THROTTLE(31, "Publishing point cloud...");
		...
		if(cloudHeaderPub_.getNumSubscribers())// only publish when someone is hearing
				{
						...
						if (GetSourceCloud::getCloud(imagePtr->image, imageDepthPtr->image, fx_, fy_, cx_, cy_, maxDepth_, minDepth_, pclCloud))
								{
										ROS_INFO_ONCE("Getting cloud...");
...
int main(int argc, char **argv)
{
		...
		dynamic_reconfigure::Server<publish_point_cloud::ppcParametersConfig> server;
		dynamic_reconfigure::Server<publish_point_cloud::ppcParametersConfig>::CallbackType f;

		f = boost::bind(&dyCallback, _1, _2);
		server.setCallback(f);

		// set up source point cloud publisher
		cloudHeaderPub_ = nh.advertise<std_msgs::Header>("pointcloud/header", 3);
		...
		ROS_INFO_ONCE("Start publishing point_cloud.\n");
