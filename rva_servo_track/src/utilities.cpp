...
void Utilities::markImage(Mat img_in, RotatedRect roi, Mat &img_out, std::vector<int> &mask_id)
{
		Mat mask(img_in.rows + 2, img_in.cols + 2, CV_8UC1, Scalar(0));
		Mat img_hsv;
		cvtColor(img_in, img_hsv, CV_BGR2HSV);
		floodFill(img_hsv, mask, roi.center, Scalar(255,255,255), 0, Scalar(20,80,80), Scalar(20,80,80), 4 | (255 << 8) | CV_FLOODFILL_FIXED_RANGE | CV_FLOODFILL_MASK_ONLY);
...
//		waitKey(10);
		...
		drawObject(roi.center.x, roi.center.y, img_out);
}
